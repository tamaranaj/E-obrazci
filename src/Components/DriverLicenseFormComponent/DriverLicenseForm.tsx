...
export const DriverLicenseForm = ({
  ...
  hideButtons,
  languageFormProps,
}: DriverLicenseFormComponentProps) => {
  const { driverLicense, updateDriverLicense, documentLanguage, tabs } =
  ...
  return (
    <div className="dlWrapper">
      <fieldset className="reasons">
        ...
        {driverLicense.reason === "" && (
          <span className="errorMessage">{errorRequired}</span>
        )}
      </fieldset>
      <fieldset className="reasons">
        <ProcedureComponent
          procedureConfig={dLicenseConfig.procedure}
          handleChange={updateDriverLicense}
          state={driverLicense.procedure}
        />
        {driverLicense.procedure === "" && (
          <span className="errorMessage">{errorRequired}</span>
        )}
      </fieldset>
      {documentLanguage === "мк" && (
        <BilingualNameComponent
          ...
          handleChange={updateDriverLicense}
          state={driverLicense.nameLanguage}
          name="nameLanguage"
        />
      )}
